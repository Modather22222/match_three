...
A casual Match-3 tile puzzle game where players swap adjacent tiles to create lines of 3 or more matching tiles. Players complete levels by reaching score targets within a limited number of moves. The game features special tiles, power-ups, a coin economy, and monetization through rewarded ads and in-app purchases.
...
**Critical principle:** The engine layer (board.dart, match_finder.dart, cascade_processor.dart, special_tile_logic.dart) must be pure Dart with ZERO Flame or Flutter dependencies. This makes it testable and reusable.
...
Use Flutter's Navigator with custom fade transitions. Every scene change goes through a transition manager that performs:
...
The transition overlay is a black `ColorFiltered` or `AnimatedOpacity` widget placed above all content using a Stack.

### 3.3 Daily Reward
On app launch (Main Menu), check if daily reward is available. If today's date differs from the stored `lastDaily` date, show a popup after 0.5s delay:
- Title: "DAILY REWARD!"
...
```
Each cell holds a `TileModel?` reference. `null` means the cell is empty (during gravity/clear operations).

### 4.4 Board Background Rendering
Draw the following behind the tiles:
1. **Dark panel**: Rounded rectangle with `Color(0, 0, 0, 0.35)`, 10px margin around the grid area, with a subtle `Color(1, 1, 1, 0.08)` border (2px).
2. **Checkerboard cells**: For each cell, draw a slightly lighter or darker rectangle. Even cells `Color(1, 1, 1, 0.06)`, odd cells `Color(1, 1, 1, 0.03)`.
...
3. **Highlight band**: Top 38% of body, color lightened by 35%, inset 3px on each side
4. **Border**: 2px stroke, color darkened by 30%
5. **Selection glow**: When selected — white 3px outer stroke at +3px growth, then a yellow-tinted 1.5px stroke at +5px growth
6. **Symbol**: Centered text character using the fallback font, size = 36% of tile size, white at 88% opacity
...
### 6.4 Swipe Threshold
`const SWIPE_THRESHOLD = 28.0` logical pixels. If the finger moves less than this distance, it counts as a tap. If more, it counts as a swipe.
...
### 7.1 Overview
The match finder scans the entire board for horizontal and vertical runs of 3+ consecutive tiles of the same type. It then classifies matches into special tile categories based on shape and length.

### 7.2 Finding Runs

#### Horizontal Runs
For each row, iterate left to right tracking a "run start" index. When the current tile type differs from the run start type (or we reach the end of the row):
...
All positions in all runs (regardless of special classification) are added to the "clear set" EXCEPT positions that will become special tiles.
...
Color bombs do NOT participate in normal color matching. When scanning for runs, skip positions that contain color bombs. Color bombs can only be activated by being swapped with another tile (or by another special tile effect).
...
Unlike board generation, refill does NOT prevent matches — matches created by refill tiles trigger the next cascade iteration, which is the desired behavior.
...
#### Wrapped Effect
When activated at position (col, row):
* Add all positions (col+dc, row+dr) where dc ∈ {-1, 0, 1}, dr ∈ {-1, 0, 1} excluding (0, 0), only if in bounds
...
* **Effect**: All tiles of the striped tile's color become striped tiles (same H/V direction), then ALL of them activate simultaneously
* **Process**:
...
When clearing tiles, if any cleared tile is itself a special tile, its effect must also trigger. This creates chain reactions.
...
* Visual style varies by state:
  ...
  * Locked: dark gray, padlock icon, disabled
...
| :--- | :--- | :--- | :--- |
| Hammer | Hammer | "Hammer ×{count}" | "Hammer {HAMMER_COST} Coin" |
| Shuffle | Shuffle | "Shuffle ×{count}" | "Shuffle {SHUFFLE_COST} Coin" |
| Extra Moves | +5 | "+5 ×{count}" | "+5 {MOVES_COST} Coin" |
...
| :--- | :--- | :--- | :--- |
| 1 | Coins | Coins | Current coin count |
| 2 | Hammer | Hammer | Current hammer stock |
| 3 | Shuffle | Shuffle | Current shuffle stock |
| 4 | +5 Moves | +5 Moves | Current moves stock |
...
| :--- | :--- | :--- |
| Hammer | "Destroys one tile on the board. Useful for breaking tough spots!" | 50 Coins |
| Shuffle | "Shuffles all tiles randomly. No move is consumed!" | 30 Coins |
| Extra Moves | "Adds 5 extra moves to your remaining move count." | 40 Coins |
...
| Not enough coins | "Not enough coins! Need {cost} Coins" | Red |
| IAP bought | "+{amount} Coins purchased!" | Green |
...
Use SharedPreferences (simple key-value) or Hive (faster for complex data). Store as a single JSON string under key "match3_save".
...
All sounds are procedurally generated as short sine-wave tones with envelope shaping. This eliminates the need for audio asset files.
...
### 17.4 Sound Toggle
A boolean soundEnabled in save data. All sound playback checks this flag before playing.
...
* **Particle Properties**:
  * Component positioned at tile center with random offset ($\pm 10$ pixels per axis).
  * Renders an expanding circle starting at radius 3px up to a random max (16–28px).
  * Color selected randomly from active tile colors plus white.
  * Alpha starts at 0.65 and fades to 0 over 0.35 seconds.
  * Self-destructs after 0.35s duration.

### 18.2 Swap Animation
Two adjacent tiles animate simultaneously during a player swap:
* **Duration**: 0.18s
* **Easing**: ease-out cubic
* **Property**: Position vector tween from current grid slot to target slot.

### 18.3 Clear Animation
Matched tiles animate simultaneously upon destruction:
* **Scale**: (1,1) $\rightarrow$ (0,0) over 0.22s with ease-in back easing.
* **Opacity**: 1.0 $\rightarrow$ 0.0 over 0.22s.
* **Cleanup**: Node removes itself from the component tree immediately after completion.

### 18.4 Gravity Animation
Existing tiles falling downward to fill empty grid slots:
* **Position**: Interpolates from current Y to destination Y.
* **Duration**: $0.10s + (0.04s \times \text{number of rows fallen})$
* **Easing**: ease-in bounce

### 18.5 Refill Animation
New tiles dropping onto the board from the upper buffer zones:
* **Position**: Interpolates from above the board boundary down to final grid position.
* **Duration**: $0.10s + (0.035s \times \text{rows to fall})$
* **Easing**: ease-in bounce
* **Stagger**: $0.03s \times \text{drop index within column}$ (creates a waterfall sequence).
* **Offset**: Initial placement features a random vertical shift of 30–80px above calculated boundary.

### 18.6 Special Tile Creation Animation
When a position transitions into a special tile type:
* New tile spawns at scale (0.4, 0.4).
* Animates to scale (1.2, 1.2) over 0.12s.
* Settles back to scale (1.0, 1.0) over 0.10s.

### 18.7 Board Entrance Animation
Executed sequentially at the start of a level:
...
Implemented via Flutter interface layer:
* Utilizes `InkWell` or `GestureDetector` coupled with an `AnimatedContainer`.
* Governs standard interaction states across press down and hover inputs.

### 18.9 Hammer Mode Visual
State feedback indicators when hammer power-up is active:
* Board frame renders a pulsing orange border (`#F39C12`, 4px thickness, 50% opacity).
* Display text updates to "TAP A TILE TO DESTROY".
* Dismissed upon valid selection target tap, direct button re-tap, or out-of-bounds tap.
...
└───────────────────────────────────────┘
```
* **Panel Configuration**: 420×370 size, background color `#16213E`, gold frame border 3px, corner roundness radius 16px, padding bounds 28px.
* **Control Layer**: Screen background overlay tinted black at 55% opacity to block standard input routing.
* **Win Event Hook Execution Sequence**:
  1. Trigger "win" audio node.
  2. Initiate haptic pulse motor for a 50ms heavy block vibration.
  3. Commit star ratings and top profile score metadata to game save database state.
  4. Distribute standard level clearing currency adjustments.
  5. Draw overlay layout.
...
└───────────────────────────────────────┘
```
* **Panel Configuration**: 420×340 size, background color `#1A1A2E`, red border outline 3px, corner roundness radius 16px.
* **Lose Event Hook Execution Sequence**:
  1. Trigger "lose" audio node.
  2. Initiate haptic pulse motor for a 25ms medium vibration.
  3. Decrement active baseline player profile heart count by 1.
  4. Write out cumulative stage currency metrics to local memory container.
  5. Draw overlay layout.
* **Rewarded Ad Execution Loop**:
  1. Draw temporary media loading blocker widget featuring a mock 3-second loader sequence bar overlay and functional skip trigger target.
  2. Completion callback logic closes modal instance container, appends `+5` integer count onto move stack register, and resets gameplay loop state flags.
  3. Close/fail exception events clear tracking parameters and re-render standard lose popup interface.
...
**Implementation:** Show a simulated ad overlay with a 3-second progress bar and a "SKIP (test)" button. In production, replace with real `AdMob RewardedAd.load()` / `.show()`.
...
**Implementation:** Show confirmation popup, then simulate instant grant. In production, replace with Google Play Billing / Apple StoreKit calls.
...
1. **Base fill:** Solid dark color `Color(0.03, 0.03, 0.07)`
2. **Vertical gradient bands:** 12 horizontal strips spanning the viewport. Each strip is a slightly different dark shade of purple/blue, creating a subtle gradient effect. Alpha: 0.35
3. **Drifting glow orbs:** Two large soft circles that slowly move:
...
4. **Floating particles:** 20 small white circles:
...
Update all particle positions each frame. Redraw the entire background every frame using `_draw()` or Flame's `render()`.
...
When the app first starts, show the main menu with a fade-in from black (0.3 seconds).
...
When an object is "destroyed," return it to the pool instead of actually freeing it. When a new object is needed, take from pool first.
...
**Note:** Coin packs are CONSUMABLE purchases (can buy multiple times). Remove Ads and Starter Pack are NON-CONSUMABLE (one-time).
...
Use a distinctive display font for titles (e.g., Bebas Neue, Syne, or Montserrat) paired with a clean body font (e.g., DM Mono, Lora, or Source Serif 4). Load via Google Fonts CDN or bundle in assets.
